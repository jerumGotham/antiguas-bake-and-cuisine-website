...
export function Header() {
  return (
    <header className="sticky top-0 z-50 border-b border-border/35 bg-card/95">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-8 md:py-5">
        ...
        <div className="flex items-start gap-4">
          ...
          >
            {siteNav.map((item) => (
              <Link
                className="border-b-2 border-transparent pb-1 text-sm font-semibold text-foreground transition hover:border-primary"
                href={item.href}
                key={item.href}
              >
                {item.label}
